...
class PositionSearchProblem(search.SearchProblem):
  ...
  def __init__(self, gameState, costFn = lambda x: 1, goal=(1,1), start=None, warn=True):
    ...
    star=self.startState

    
    f = open('maze.P', 'w')
    f.write('/* Maze height='+str(height)+', Maze width= '+str(width)+'*/\n')
        ...
        for i in range(1,height-1):
            ...
            if(self.walls[j][i]==False):
                ...
                if(self.walls[j-1][i]==False):
                    f.write('s(c'+str(j)+'_'+str(i)+', c'+str(j-1)+'_'+str(i)+').')
                    f.write('\n')
                ...
                if(self.walls[j][i-1]==False):
                    f.write('s(c'+str(j)+'_'+str(i)+', c'+str(j)+'_'+str(i-1)+').')
                    f.write('\n')

                if(self.walls[j][i+1]==False):
                    f.write('s(c'+str(j)+'_'+str(i)+', c'+str(j)+'_'+str(i+1)+').')
                    f.write('\n')
    f.write("goal(c"+str(self.goal[0])+"_"+str(self.goal[1])+").\n")                
