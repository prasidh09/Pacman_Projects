...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    #print "Noise Dist"+str(noisyDistance)
    #print "Emission model" + str(emissionModel)
    #print "Pacman pos"+str(pacmanPosition)
    ...
    for p in self.legalPositions:
      ...
      if emissionModel[trueDistance] > 0:
          #Beliefs get pushed through for each transition and updated
          # at each time step with current beliefs
  ...
  def elapseTime(self, gameState):
    ...
    for p in self.legalPositions:
        ...
        PositionDistribution = self.getPositionDistribution(gameState)
        for position, probability in PositionDistribution.items():
            beliefs[position] = probability * self.beliefs[p] + beliefs[position]
