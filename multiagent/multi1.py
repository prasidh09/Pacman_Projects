...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        if newScaredTimes == [0] and newGhostDist < 3:
            score -= 7*(oldGhostDist - newGhostDist)
        minOld = minNew = 0
        ...
        for food in newFoodList:
            ...
            if flag == 0:
                ...
                if dist < minNew:
                    minNew = dist
        if newGhostDist > 2:
            score -= 10*(minNew - minOld)
            score -= 1000*(len(newFoodList) - len(oldFoodList))
        else:
            score -= 5*(minNew - minOld)
            score -= 6*(len(newFoodList) - len(oldFoodList))
